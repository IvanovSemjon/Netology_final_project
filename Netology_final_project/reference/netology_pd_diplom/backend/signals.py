...
from backend.tasks.celery_tasks import send_order_status_email_task, send_generic_email_task
...
def password_reset_token_created_handler(sender, instance, reset_password_token, **kwargs):
    """Отправка письма со сбросом пароля через Celery."""
...
def order_status_changed_handler(sender, instance, created, **kwargs):
    """Асинхронная отправка уведомлений при изменении статуса заказа."""
