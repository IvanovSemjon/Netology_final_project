...
class CustomUserAdmin(UserAdmin):
    """
    Панель управления пользователями с аватаром.
    """
    model = User
    list_display = ('email', 'first_name', 'last_name', 'type', 'is_active', 'is_staff', 'date_joined', 'avatar_tag')
    ...
    fieldsets = (
        ...
        ('Личная информация', {'fields': ('first_name', 'last_name', 'company', 'position', 'avatar', 'avatar_tag')}),
        ('Разрешения', {
    ...
    def avatar_tag(self, obj):
        if obj.avatar:
            return format_html('<img src="{}" style="height: 50px;"/>', obj.avatar.url)
        return "-"
...
class ProductInfoAdmin(admin.ModelAdmin):
    list_display = ('product', 'shop', 'model', 'price', 'price_rrc', 'quantity', 'availability_status', 'image_tag')
...
# Переименовываем стандартные модели
